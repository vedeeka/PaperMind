...
from ingestion.pdf_ingestion import process_pdf, list_indexed_papers, delete_paper, STORAGE_DIR
from agent_service import get_sub_questions, run_research_pipeline, compare_papers
...
def get_status():
    ...
    return {
        ...
        "api_key_masked": f"{current_key[:4]}...{current_key[-4:]}" if len(current_key) > 8 else ("Configured" if current_key else "Not Set"),
        "papers": papers
    }
...
@app.post("/api/reset")
def reset_session():
    """Wipes all documents from ChromaDB and starts a clean session."""
    reset_database()
    return {
...
def get_paper_chunks(filename: str):
    try:
        data = get_collection().get(where={"source": filename}, include=["documents", "metadatas"])
        chunks = []
...
def get_paper_pdf(filename: str):
    ...
    if not os.path.exists(target_path):
        # Check workspace root or deepresearch
        root_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), filename)
        deep_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "deepresearch", "papers", filename)
...
async def upload_pdf(file: UploadFile = File(...)):
    ...
    try:
        result = process_pdf(temp_path, file.filename, save_copy=True)
        return {
...
def index_sample_papers():
    """Indexes paper1.pdf and paper2.pdf bundled with the repository."""
    base_dir = os.path.dirname(os.path.abspath(__file__))
    candidates = [
        (os.path.join(base_dir, "paper1.pdf"), "Attention_Is_All_You_Need.pdf"),
        (os.path.join(base_dir, "deepresearch", "papers", "paper2.pdf"), "PaperMind_Agentic_RAG_Architecture.pdf")
    ]
    ...
    for path, target_name in candidates:
        if os.path.exists(path):
            try:
                res = process_pdf(path, target_name, save_copy=True)
                indexed.append(res)
            except Exception as e:
                print(f"Error indexing sample {path}: {e}")
                
    return {
        "message": f"Successfully indexed {len(indexed)} sample research papers.",
        "indexed": indexed,
        "total_chunks": get_collection().count()
    }
...
def search_endpoint(req: SearchRequest):
    results = semantic_search(req.query, n_results=req.top_k)
    return {
        "query": req.query,
        "count": len(results),
        "results": results
...
@app.get("/question")
def legacy_question(query: str = Query(...)):
...
def breakdown_endpoint(req: BreakdownRequest):
    sub_questions = get_sub_questions(req.question, req.api_key)
    return {
...
def compare_endpoint(req: CompareRequest):
    if not req.papers:
        raise HTTPException(status_code=400, detail="Please select at least 1 paper to compare.")
    try:
        result = compare_papers(req.papers, req.api_key)
        return result
    except Exception as e:
